...
const ViewArticle = () => {
  ...
  const fetchCategory = async () => {
    setLoading(true);
    try {
      const res = await authService.getCategoryByIdForSingle(
        user.accessToken,
        id,
      );
      console.log('res', res.data.data.category);

      setArticleData(res.data?.data.category);
    } catch (error) {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        <div className="rounded-2xl overflow-hidden bg-white border border-gray-200 shadow-sm hover:shadow-md transition duration-300">
          {/* Big Image */}
