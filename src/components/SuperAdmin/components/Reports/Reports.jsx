...
const Reports = () => {
  ...
  const columns = {
    stock: ['Article', 'Category Name', 'Color','Size','Type','Quality', 'Factory', 'Warehouse','Stock in Warehouse', 'Total Quantity'],
    sales: ['Sales Order No', 'Order Date', 'Customer', 'Article', 'Quantity'],
    products: ['Production No.', 'Article', 'Color', 'Size','Type','Quality', 'Factory Name', 'Production Quantity', 'Dispatched Quantity'],
    warehouse: ['Warehouse Name', 'Location', 'Total Quantity'],
    customer: ['Customer', 'Article', 'City', 'Total Quantity'],
  };

  const rowMappers = {
    stock: (row) => [row.article, row.categoryCode,row.color,row.size,row.type,row.quality, row.factory, row.warehouse, row.stockAtWarehouse, row.totalQuantity],
    sales: (row) => [row.salesOrderNo, row.orderDate, row.customer, row.article, row.quantity],
    products: (row) => [row.productionNo, row.article, row.category.color,row.category.size, row.category.type, row.category.quality, row.factory?.name,  row.productionQuantity, row.dispatchedQuantity],
  ...
  const fetchStockReport = async () => {
    try {
      setLoading(true);
      const res = await reportService.stockReport(user.accessToken, pagination.stock.currentPage, 10);
  ...
  const fetchSalesReport = async () => {
    try {
      setLoading(true);
      const res = await reportService.salesReport(user.accessToken, pagination.sales.currentPage, 5);
  ...
  const fetchProductionReport = async () => {
    try {
      setLoading(true);
      const res = await reportService.productionReport(user.accessToken, pagination.products.currentPage, 5);
      setProductionReport(res?.products || []);
      setPagination(prev => ({
        ...prev,
        products: { ...prev.products, totalPages: res?.pagination?.totalPages || 1 },
      }));
    } catch (err) {
  ...
  const fetchWarehouseReport = async () => {
    try {
      setLoading(true);
      const res = await reportService.warehouseReport(user.accessToken, pagination.warehouse.currentPage, 5);
      setWarehouseReport(res?.data?.data || []);
      setPagination(prev => ({
        ...prev,
        warehouse: { ...prev.warehouse, totalPages: res?.data?.pagination?.totalPages || 1 },
      }));
    } catch (err) {
  ...
  const fetchCustomerReport = async () => {
    try {
      setLoading(true);
      const res = await reportService.customerReport(user.accessToken, pagination.customer.currentPage, 5);
      setCustomerReport(res?.data || []);
      setPagination(prev => ({
        ...prev,
        customer: { ...prev.customer, totalPages: res?.pagination?.totalPages || 1 },
      }));
    } catch (err) {
          ...
          <thead className="bg-gray-50 text-gray-600 uppercase text-xs">
            <tr>
              {columns[activeReport].map((col, i) => (
                <th key={i} className="p-4 whitespace-nowrap">{col}</th>
              ))}
            </tr>
          </thead>
        ...
        <div className="flex justify-between items-center p-4 border-t text-sm text-gray-600">
          <button
            ...
            onClick={() =>
              setPagination(prev => ({
                ...prev,
                [activeReport]: { ...prev[activeReport], currentPage: prev[activeReport].currentPage - 1 },
              }))
            }
