...
import { FiEye, FiTrash2, FiFilter, FiX } from 'react-icons/fi'
...
const FactoryManagement = () => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await factoryService.DeleteFactory(user.accessToken, selectId)
      setFactoryData(prev => prev.filter(item => item._id !== selectId))
      toast.success(res?.data?.message || 'factory deleted successfully')
      setSelectId(null)
      ...
      <div className='flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4'>
        ...
        <div className='flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 w-full sm:w-auto mt-4 sm:mt-0'>
          ...
          <button
            ...
            onClick={() => navigate('/factory-management/CreateFactory')}
          >
            Create Factory
          </button>
        </div>
              ...
              <tr key={index} className='border-t hover:bg-gray-50'>
                ...
                <td className='p-3 flex justify-center gap-3'>
                  <button
                    ...
                    onClick={() => navigate(`/factory-details/${entry._id}`)}
                  >
                    <FiEye size={16} />
                  </button>
                  <button
                    className='text-blue-600 hover:text-blue-800'
                    onClick={() => navigate(`/factory-edit/${entry._id}`)}
                  >
