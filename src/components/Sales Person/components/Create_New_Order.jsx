    ...
    try {
      ...
      const mappedData = rawData.reduce((acc, item, index) => {
        ...
        const categoryObj = {
          ...
          color: item.color,
          size: item.size,
          type: [item.type], // UI expects array
          quality: [item.quality], // UI expects array
          image: item.image || [],
          totalAvailableQty: item.Total_Available,

          // availableQty: item.Total_Available
        }
                ...
                <div className='grid grid-cols-1 md:grid-cols-3 gap-3'>
                  ...
                  >
                    ...
                    {getTypesForColorSize(
                      ...
                      item.size,
                      item.type
                    ).map(totalAvailableQty => (
                      <option key={totalAvailableQty} value={totalAvailableQty}>
                        {totalAvailableQty}
                      </option>
                    ))}
                  </select>
