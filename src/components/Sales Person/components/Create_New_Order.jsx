  ...
  const CategoryDashboard = () => {
    ...
    const fetchProducts = async (pageNo = 1, debounceValue = '') => {
      ...
      try {
        ...
        const mappedData = rawData.reduce((acc, item, index) => {
          const existing = acc.find(p => p.article === item.article)
          ...
          const categoryObj = {
            ...
            image: item.image || [],
            totalAvailableQty: item.Total_Available // ✅ correct
          }

          if (existing) {
            existing.category.push(categoryObj)
          } else {
            acc.push({
              _id: `${item.article}-${index}`, // fake id for UI
        ...
        if (pageNo === 1) setProducts(mappedData)
    ...
    // Get total available quantity options for selected article, color, size, and type
    
  const getTotalAvailableForType = (articleId, color, size, type) => {
    const product = products.find(p => p._id === articleId)
    if (!product || !color || !size || !type) return []

    const matches = product.category.filter(
      cat =>
        cat.color === color &&
        cat.size === size &&
        cat.type?.includes(type)
    )
    // return as array so <select> works
    return Array.from(
      new Set(matches.map(m => m.totalAvailableQty).filter(Boolean))
    )
  }
    ...
    const getQualitiesForColorSizeType = (
      ...
      const product = products.find(p => p._id === articleId)
      if (!product || !selectedColor || !selectedSize || !selectedType) return []
                ...
                >
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-3 gap-3'>
                    ...
                    >
                      ...
                      {getSizesForColor(item.articleId, item.color).map(size => (
                    ...
                    <select
    value={item.totalAvailableQty}
    onChange={e => {
      const updated = [...formData.items]
      updated[idx].totalAvailableQty = e.target.value
      updated[idx].quality = ''
      setFormData(prev => ({ ...prev, items: updated }))
    }}
    className='w-full border rounded px-3 py-2'
    disabled={!item.type}
  >
    <option value=''>Select Total Available</option>
    {getTotalAvailableForType(
      item.articleId,
      item.color,
      item.size,
      item.type
